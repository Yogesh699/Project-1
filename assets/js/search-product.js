function generateProductsSearch(element){
    let singleProduct = `
...
document.getElementById("search-form").addEventListener("submit", (event)=>{
    ...
    let searchItem = event.currentTarget.search.value
    fetch('https://fakestoreapi.com/products')
    .then(res=>res.json())
    .then(data=> {
        let searchData = data.filter(item => item.title.toLowerCase().includes(searchItem.toLowerCase()));

        mainDiv.innerHTML = ""
        searchData.forEach(element => {
           mainDiv.innerHTML +=  generateProductsSearch(element)
        })
    })
})